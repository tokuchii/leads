    ...
    <section id="news" class="bg-white py-10">
        <div class="container mx-auto px-12 mt-22">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div v-for="news in newsList.slice(0, 3)" :key="news.id" class="bg-[#2E7D32] rounded-2xl overflow-hidden shadow-lg flex flex-col h-full">
                    ...
                    <div class="p-6 flex flex-col flex-1">
                        ...
                        <button @click="$emit('show-news-article', news)" class="news-learnmore-btn cursor-pointer self-start text-xs sm:text-sm md:text-base">LEARN MORE.</button>
                    </div>
                </div>
            </div>
            <div class="text-center mt-10">
    <button
        @click="$emit('show-featured-news')"
