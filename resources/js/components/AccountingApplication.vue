    ...
    <div class="flex-1 flex flex-col items-center justify-center py-10 px-6">
      ...
      <template v-else>
        ...
        <div class="mb-8 w-full flex justify-center items-center">
          <span class="bg-gray-100 text-xs px-4 py-3 rounded-full text-[#003D1FAD] shadow-md font-bold">VACANCY / CAREERS</span>
        </div>
        <div class="w-full max-w-6xl flex flex-col md:flex-row gap-10 items-stretch">
          <!-- Left: Job Details-->
          <div class="bg-[#003D1F] text-white p-6 flex flex-col w-full md:w-1/2 justify-between rounded-3xl shadow-2xl mb-8 md:mb-0 min-h">
            <div>
              <div class="text-based mb-2">Full time</div>
              <div class="text-2xl font-bold mb-2">Accounting Staff</div>
              <div class="text-based mb-2">Graduate of BS Accountancy; Proficient in Microsoft Office applications particularly MS Word and MS Excel</div>
          ...
          <div class="p-8 flex flex-col w-full md:w-1/2 justify-center h-full">
            ...
            <form class="flex flex-col gap-4" @submit.prevent="submitForm">
              ...
              <input type="text" placeholder="Your Position*" value="Accounting Staff" readonly class="rounded-full bg-[#F4F4F4] shadow-md placeholder:italic px-4 py-3 border border-gray-200 focus:outline-none focus:ring-0 text-black" />
              <textarea placeholder="Enter your message" rows="4" class="rounded-2xl bg-[#F4F4F4] placeholder:italic px-4 py-3 border border-gray-200 focus:shadow-md focus:outline-none focus:ring-0 resize-none transition-all duration-200"></textarea>
              <div>
                <input id="resume-upload" type="file" class="hidden" @change="handleFileChange" />
                <label for="resume-upload" class="inline-block rounded-full bg-[#F4F4F4] px-4 py-1 mb-2 text-gray-400 italic font-medium cursor-pointer hover:shadow-md transition-all duration-200">Add File Here*</label>
                <span v-if="selectedFileName" class="block mt-1 text-sm text-gray-700">{{ selectedFileName }}</span>
              </div>
