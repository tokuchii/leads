<template>
  <div class="relative w-full min-h-[800px] flex items-center justify-center sugarcane-bg-outer overflow-hidden">
    <!-- Background image -->
    <div class="absolute inset-0 bg-cover bg-center bg-no-repeat z-0 sugarcane-bg-img"></div>
    <!-- White overlay -->
      ...
      <div class="relative" :class="[activeTabBg, 'pt-6 rounded-4xl']" style="min-height: 480px;">
        <div class="bg-[#FFFFFF] rounded-4xl p-4 sm:p-8 lg:p-14 text-back shadow-xl" style="min-height: 600px;">
          ...
          <div v-if="activeTab === 'Herbicide' && sugarcaneHerbicideProducts.length" class="space-y-8">
            <div v-for="product in sugarcaneHerbicideProducts" :key="product.id" class="flex flex-col md:flex-row bg-white rounded-3xl overflow-hidden">
          ...
          <div v-if="activeTab === 'Biostimulant' && sugarcaneBiostimulantProducts.length" class="space-y-8">
            <div v-for="product in sugarcaneBiostimulantProducts" :key="product.id" class="flex flex-col md:flex-row bg-white rounded-3xl overflow-hidden">
              <div class="flex-shrink-0 flex items-center justify-center md:w-1/2 pt-12">
                <img v-if="product.image1_url" :src="product.image1_url" alt="Image 1" class="w-full h-full" />
              </div>
              <div class="flex-1 p-6 md:p-8">
...
export default {
  ...
  computed: {
    ...
    sugarcaneHerbicideProducts() {
      return this.products
        .filter(p => p.category === 'Sugarcane' && p.type === 'Herbicide')
        .sort((a, b) => a.id - b.id);
    },
    sugarcaneBiostimulantProducts() {
      return this.products
        .filter(p => p.category === 'Sugarcane' && p.type === 'Biostimulant')
        .sort((a, b) => a.id - b.id);
    },
  },
  methods: {
